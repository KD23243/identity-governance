...
/**
 * Executor that collects the password input onto the flow user.
 * <p>
 * The actual credential provisioning has been consolidated into {@link UserProvisioningExecutor}, which sets the
 * password at the terminal node of the flow. This executor is retained only to capture the password field for the
 * UI step (and to prompt for it when missing); it performs no credential update itself.
 */
public class PasswordProvisioningExecutor extends AuthenticationExecutor {
    ...
    public ExecutorResponse execute(FlowExecutionContext context) {
        ...
        } else if (MapUtils.isEmpty(context.getFlowUser().getUserCredentials())) {
